...
(function( root, factory ) {
    ...
    define('runtime/runtime',[
        ...
        Runtime.create = function( opts, comment ) {
            var type, runtime;
    
            comment = comment || Runtime.comment;
            $.each( comment.split( /\s*,\s*/g ), function() {
                if ( factories[ this ] ) {
                    type = this;
                    return false;
                }
    ...
    define('widgets/runtime',[
        ...
        return Uploader.register({
            ...
            predictRuntmeType: function() {
                var comment = this.options.runtimeOrder || Runtime.comment,
                    type = this.type,
                    i, len;
    
                if ( !type ) {
                    comment = comment.split( /\s*,\s*/g );
    
                    for ( i = 0, len = comment.length; i < len; i++ ) {
                        if ( Runtime.hasRuntime( comment[ i ] ) ) {
                            this.type = type = comment[ i ];
                            break;
                        }
                    }
                }
